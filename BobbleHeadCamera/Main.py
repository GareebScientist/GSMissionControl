...
class ImageCombinationApp:
    def __init__(self):
        ...
        self.button_select_angry_image = tk.Button(
            self.window, text="Select Angry Image", command=self.select_angry_image
        )
        self.button_select_angry_image.pack(pady=10)

        self.button_combine_images = tk.Button(
            self.window, text="Combine Images", command=self.display_images
        )
        self.button_combine_images.pack(pady=10)

        self.get_microphone_inputs()

        self.microphone_dropdown = tk.OptionMenu(
            self.window,
            self.microphone_variable,
            *self.microphone_inputs,
        ...
        self.gradient_bar = self.gradient_canvas.create_rectangle(
            0, 0, 0, 20, fill="green"
        )

        self.bobbing_slider = tk.Scale(
            self.window,
            from_=0,
            to=200,
            orient=tk.HORIZONTAL,
            length=200,
            command=self.update_bobbing_threshold,
        )
        ...
        self.checkbutton_angry_mode = tk.Checkbutton(
            self.window,
            text="Angry Mode",
            variable=self.angry_mode_var,
            command=self.toggle_angry_mode,
        )
    ...
    def display_images(self):
        ...
        if combined_image is not None:
            pygame_thread = threading.Thread(
                target=self.display_pygame_window, args=(combined_image,)
            )
            pygame_thread.start()
    ...
    def combine_images(self):
        if all(image is not None for image in self.images):
            max_height = max(
                image.shape[0] for image in self.images if image is not None
            )
            max_width = max(
                image.shape[1] for image in self.images if image is not None
            )
            ...
            if self.angry_mode and self.angry_image is not None:
                combined_image[
                    self.image2_position : self.image2_position
                    + self.angry_image.shape[0],
                    : self.angry_image.shape[1],
                ] = self.angry_image
        ...
        for image in self.images:
            if image is not None:
                ...
                surface = pygame.image.fromstring(
                    image.tobytes(), image.shape[1::-1], "RGBA"
                )
        ...
        while running:
            ...
            for i, surface in enumerate(surfaces):
                if i != 3 or (i == 3 and self.angry_mode):
                    ...
                    if (
                        i == 1 or i == 3
                    ):  # Adjust the position of image 2 and angry image
    ...
    def start_audio_processing(self, microphone_name):
        if microphone_name != "Select Microphone":
            selected_microphone_index = (
                self.microphone_inputs.index(microphone_name) - 1
            )
            self.stop_audio_processing()
            audio_thread = threading.Thread(
                target=self.audio_processing,
                args=(selected_microphone_index,),
                daemon=True,
            )
            audio_thread.start()
    ...
    def audio_processing(self, selected_microphone_index):
        ...
        while self.processing:
            ...
            self.gradient_canvas.coords(
                self.gradient_bar, 0, 0, rms * 2, 20
            )  # Multiplied by 2 to get visible change
