    ...
    if countdown_thread:
        ...
        if utc_time_str:  # If a UTC time was entered
            try:
                ...
                if (
                    t < 0
                ):  # If the entered time is in the past, display an error and return
...
Label(root, text="Enter UTC time (YYYY-MM-DD HH:MM:SS):", font=("Helvetica", 14)).grid(
    row=0, column=0, columnspan=5
)
utc_entry = Entry(root, font=("Helvetica", 12))
utc_entry.grid(row=1, column=0, columnspan=5)

Label(root, text="OR Enter seconds:", font=("Helvetica", 14)).grid(
...
toggle_button = Button(
    root, text="Start Timer", command=toggle_countdown, font=("Helvetica", 14)
)
...
for i in range(1, 6):
    Button(
        root,
        text=f"+{i} sec",
        command=lambda i=i: countdown_thread.increase_time(i)
        if countdown_thread
        else None,
        font=("Helvetica", 14),
...
for i in range(1, 6):
    Button(
        root,
        text=f"-{i} sec",
        command=lambda i=i: countdown_thread.decrease_time(i)
        if countdown_thread
        else None,
        font=("Helvetica", 14),
    ).grid(row=6, column=i - 1)

Label(root, textvariable=time_str, font=("Helvetica", 20)).grid(
